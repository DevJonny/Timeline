...
**1. Three coordinate systems, in a chain.** Data is authored *historically*
(`-3300` is 3300 BCE, and **there is no year 0**); the maths needs a
*continuous astronomical decimal year* (1 BCE is 0); the scale and the zoom
transform run on an *axis coordinate*, which is that decimal year with the
empty ages squeezed out of it (see 6).
...
`src/lib/time.ts` owns the first hop, `src/lib/collapse.ts` the second.
Everything entering the axis goes through both; everything shown to a human
comes back through both. **Never render a raw axis coordinate** — which now
means never formatting `visible` or a `Resolved.t0` without `axisMap.from`
first. The two are equal whenever nothing is collapsed, which is every focused
timeline shipped so far, so this is exactly the kind of mistake that passes
...
**6. The axis is broken where nothing happened.** The Palaeolithic, Mesolithic
and Neolithic hold no entries and cover 99.6% of the main timeline's 3.3
million years. `src/lib/collapse.ts` compresses an age holding nothing to a
stub, which takes the main axis from 3.3 Myr to about 5,600 years and gives the
populated stretch 94% of the screen instead of 0.4%.

Four consequences, in the order they will catch you out:

- **Emptiness is measured against *all* entries, never a filtered set** — the
  same rule as the domain, for the same reason. A filter-driven break would
  re-scale the axis on every chip toggle. Regression test: type a broad query
  and confirm the tick positions do not move.
- **An age is empty when no *non-age* entry overlaps it.** An age containing
  only another age is still empty of anything a reader came for.
- **Being empty is not enough; it must dominate.** Below
  `COLLAPSE_MIN_SHARE` of the populated span an empty age is left alone, or
  Ancient Greece would break its axis over a fifty-year chapter.
- **The gate scale is now derived for both kinds of timeline**, from the axis
  domain against the constant `GATE_REFERENCE_SPAN` — because the main
  timeline's own span is no longer 3.3 Myr and can no longer serve as the
  reference. Nothing is the unscaled case any more.

Ticks are suppressed inside a break (`generateAxisTicks`) and a hatched band is
drawn over it, because that is the one place a constant distance stops meaning
...
- **Zoom depth and the importance gate are both relative to the span.**
  ...
  shows about four months, and `gateScaleFor` multiplies `k` before the gate
  ...
  exactly, which `tests/focus.test.ts` asserts. Both read the **axis** domain,
  after collapse — a focus with an empty chapter has genuinely fewer years on
  screen and has earned the detail that goes with them.
- **Importance 1 inside a focus belongs to its chapters** — its own `age` spans,
  which become the era rail. Elsewhere importance 1 is closed. A chapter with
  nothing in it yet gets collapsed like any other empty age, and un-collapses on
  its own once someone writes the entries.
